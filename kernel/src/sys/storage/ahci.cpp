...
namespace AHCI{
    ...
    void Port::Configure() {
        ...
        void* fisBase = GlobalAllocator.RequestPage(); // Frame Information Structure
        hbaPort->fisBaseAddress = (uint32_t)(uint64_t) fisBase;
        hbaPort->fisBaseAddressUpper = (uint32_t)((uint64_t)newBase >> 32);
    ...
    bool Port::Read(uint64_t sector, uint32_t sectorCount, void* buffer) {
        ...
        hbaPort->interruptStatus = (uint32_t) -1; // Clear pending interrupts
        ...
        while(true) {
            ...
            if(hbaPort->interruptStatus & HVA_PXIS_TFES) {
                return false;
            }
        }
        return true;
